...
#Gender and Name 
userChoice = input("Would you like to your character to be Male, Female, or Randomly picked? (M/F/R) ")

if userChoice == "M":

    sex = "Male" #setting the sex of the character to male 

    with open('male_names.txt') as f: #opens file with male names
    ...
    with open('female_names.txt') as f: #opens file with male names
        lines = f.readlines() #picks randomly from the list of names
    firstName = lines[secrets.randbelow(len(lines))].strip() #strips the the newline from the name
...
#Age and Birth Year
yearChoice = input("Would you like to set the year or use the current year? (Set/Current) ")

if yearChoice == "Set":
    currentYear = input("Set the year: ") #letting the user set the year

elif yearChoice == "Current":
    currentYear = 2025 #the current year
 
age = secrets.randbelow(80 - 14 + 1) + 14 #generating a age between 14 and 80
